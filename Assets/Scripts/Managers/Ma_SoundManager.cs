...
public class Ma_SoundManager : MonoBehaviour, ICoroutineControl
{
    /// <summary>
    ...
    private class SoundManagerDebugLogger
    {
        public void SoundEntries_NoneDefined()
            => Debug.LogWarning($"No sound entries are defined for the {nameof(Ma_SoundManager)}. Won't be able to play any sounds.");

        public void SoundEntries_SomeNotDefined(List<GameSound> typesWithoutEntry)
            => Debug.LogWarning($"{nameof(Ma_SoundManager)} initialization didn't find any valid sound entries for the following sound types (these sounds cannot play): " +
